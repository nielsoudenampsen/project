...
@login_required
def home():
    favorites = json.loads(db.execute(
        "SELECT favorites FROM users WHERE id = ?", session["user_id"])[0]["favorites"])
    search = request.args.get('search')
    recipes = lookup_recipes(0, 1000, "", str(search))
    #top3_favorites = list(favorites.items())[0][1].get('img')
    # top3_favorites=top3_favorites)
...
def favorite(id):
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET favorites = ? WHERE id = ?",
                   json.dumps(session["favorites"]), session["user_id"])
...
def login():
    ...
    if request.method == "POST":
        ...
        try:
            session["favorites"] = json.loads(db.execute(
                "SELECT favorites FROM users WHERE id = ?", session["user_id"])[0]["favorites"])
        except:
            session["favorites"] = {}
            db.execute("UPDATE users SET favorites = ? WHERE id = ?",
...
def register():
    if request.method == "POST":
        ...
        db.execute("INSERT INTO users (name,hash) VALUES (?,?)", username, generate_password_hash(
    ...
    if request.method == "POST":
        ...
        if password != confirm:
            ...
            db.execute("UPDATE users SET hash = ? WHERE id = ?", generate_password_hash(
                password=password, method='pbkdf2:sha256', salt_length=8), session["user_id"])
